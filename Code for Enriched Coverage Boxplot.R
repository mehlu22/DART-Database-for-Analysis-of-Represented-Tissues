...
load("~/ISB/Coding/Results_Enriched_mastertable.RData")
...
boxplot(GO_BP, GO_CC, REACTOME, KEGG, BIOCARTA, GO_MF,
        at = c(1,2,3,4,5,6),
...
title(main="Percent Enriched Coverage by Gene Set Collection", line=1.5, cex.main=1)

#LABEL MEDIAN
#Round all percent gene set collections to 2 digits
#e <- round(as.numeric(GO_BP),digits=3)
#f <- round(as.numeric(GO_CC),digits=3)
#g <- round(as.numeric(REACTOME),digits=3)
#h <- round(as.numeric(KEGG),digits=3)
#i <- round(as.numeric(BIOCARTA),digits=3)
#j <- round(as.numeric(GO_MF),digits=3)

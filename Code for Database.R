...
#Load Tissue Lists
filename <- (read.table(file = "/Users/Admin/Desktop/R_course ISB Project/Code/HPA_Tissue_Lists/HPA_Tissues_10.1/HPA_UrinaryBladder_Enriched.tsv", sep = "\t", header = TRUE))
...
for (i in 1:length(tissue)) {
  twotgtable_db$Tissue[i]<-"Urinary Bladder"
}
#Fill gene name
for (i in 1:length(tissue)) {
  twotgtable_db$Gene[i]<-tissue[i]
}
...
#GO_BP
for (i in 1:length(tissue)) { 
  x <- intersect(tissue[i], collectionGO:BP) 
...
write.csv(tgtable_db, "./HPA_Tissues_10.1/Enrched_gene_names.csv")
#save(tgtable_db, file="~/ISB/Coding/tissueenrichedgenestable_db_updated.RData")
...
#Load Tissue Lists
filename <- (read.table(file = "/Users/Admin/Desktop/R_course ISB Project/Code/HPA_Tissue_Lists/HPA_Tissues_10.1/HPA_AdrenalGland_Elevated.tsv", sep = "\t", header = TRUE))
tissue <- as.character(filename$Gene)
